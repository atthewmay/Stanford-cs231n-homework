...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        h, cache_1 = affine_relu_forward(X,self.params["W1"],self.params["b1"])
        out, cache_2 = affine_forward(h,self.params["W2"],self.params["b2"])
        # Note Idk if they want scores to be before sm or after?
        scores = out
        ...
        dh, grads["W2"], grads["b2"] = affine_backward(dL_dscore,cache_2)
        dx, grads["W1"], grads["b1"] = affine_relu_backward(dh,cache_1)

        for k,v in grads.items():
            if "b" not in k:
                v += self.reg*self.params[k]
                loss += 0.5*self.reg*np.sum(self.params[k]*self.params[k])
                # Turns out they don't regularize weights. Right. 
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        last_dim = input_dim
        w_dims = hidden_dims + [num_classes]
    ...
    def loss(self, X, y=None):
        ...
        #For the final layer we do just affine
        scores, cache_h = affine_forward(h,self.params["W"+str(self.num_layers)],self.params["b"+str(self.num_layers)])
        caches.append(cache_h)
        ...
        for k,v in grads.items():
            if "W" in k:
                v += self.reg*self.params[k]
                loss += 0.5*self.reg*np.sum(self.params[k]*self.params[k])
                # Turns out they don't regularize biases. Right. 
    ...
    def affine_bn_relu_forward(self,x,i):
        ...
        a_hat, bn_cache = batchnorm_forward(a,gamma,beta,self.bn_params[i-1])
...
#                 h, cache_h = affine_relu_forward(x_hat,self.params["W"+str(i)],self.params["b"+str(i)])
#                 caches.append(cache_h)
#                 bn_caches.append(cache_bn)
